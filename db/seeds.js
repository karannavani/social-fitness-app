...
// GLOBAL VARIABLE
const numOfUsers = 60;
const numOrigExPlans = 50;
const totalExercisePlans = 500;

const today = moment().unix();
const dayInSeconds = 86400;

const fixedUserIds = ['5b91752666708bc8b1622705', '5b91752666708bc8b1622706', '5b91752666708bc8b1622707'];
const randomUserIds = randomIdArray(numOfUsers);

const userIds =[ ...fixedUserIds, ...randomUserIds] ;
...
function randomIdArray(totalIds) {
  const idArray = [];
  for(let i = 0; i <= totalIds; i++){
    idArray.push(createId());
  }
...
function randomUserId(){
  const randomIndex = Math.floor(Math.random() * numOfUsers);
...
/////////////////////////////////////////////////////////////////////
const exerciseIds = randomIdArray(totalExercisePlans);
console.log('the exerciseIds are ', exerciseIds);
// [
//
...
function createActiveExercisePlan(){
  ...
  for(let i = 0; i < numOfUsers; i++ ) {
    console.log('userIds is', userIds[i]);
    plan.push({
...
function randomExerciseId(){
  const randomIndex =  Math.floor(Math.random() * numOfUsers + numOrigExPlans) + numOfUsers;
...
function createOrigionalHistoricExercisePlans(){
  const plan = [];
  for(let i = numOfUsers; i < numOfUsers + numOrigExPlans; i++ ) {
    plan.push({
...
// returns an adopted historic exercisprogram with a random userId
function createAdoptedHistoricExercisePlans(){
  const plan = [];
  for(let i = numOfUsers + numOrigExPlans; i < totalExercisePlans; i++ ) {
    plan.push({
      day1: createHistoricExerciseDay(),
      day2: createHistoricExerciseDay(),
      day3: createHistoricExerciseDay(),
